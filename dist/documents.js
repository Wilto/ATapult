...
const getDocuments = async (agent, publicationUri) => {
  ...
  return docs.data.records.filter((r) => r.value.site === publicationUri).map((r) => ({ ...r.value, rkey: r.uri.split("/").pop() }));
};
...
export const createOrUpdateDocuments = async (agent, publicationUri, docs) => {
  ...
  for (const newDoc of docs) {
    ...
    } else if (["title", "description", "textContent"].some(
      (field) => oldDoc[field] !== newDoc[field] || oldDoc.publishedAt !== newDoc.publishedAt.toISOString()
    )) {
